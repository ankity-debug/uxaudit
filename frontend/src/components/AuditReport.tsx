...
export const AuditReport: React.FC<AuditReportProps> = ({ data }) => {
  ...
  const generatePDFBlob = (): Blob => {
    ...
    const companyName = getPlatformName();
    ...
    const categories = [
      { name: 'Heuristics', score: data.scores.heuristics, color: [239, 65, 113], position: { x: 0, y: 0 } },
      { name: 'UX Laws', score: data.scores.uxLaws, color: [34, 197, 94], position: { x: 1, y: 0 } },
      { name: 'Copy', score: data.scores.copywriting, color: [239, 68, 68], position: { x: 0, y: 1 } },
      { name: 'A11y', score: data.scores.accessibility, color: [59, 130, 246], position: { x: 1, y: 1 } }
    ];
    ...
    const executiveSummary = data.summary.length > 200 ? data.summary.substring(0, 200) + '...' : data.summary;
